...
#' stdz, Standardise a set of data
#'
#' Function takes a vector and scales it so the mean is 0 and the standard deviation is 1. It is 
#' intended to be 
#' used with the \code{ply} package that implement the split-apply-combine pattern in R
#'
#' @param x, An R object. Currently there is only a method for numeric vectors.
...
#' @note nn
#' @export
#' @seealso \code{\link{minMax}} (scale so min=0 and max=1)
#' @references \url{}
#' @examples  
#' x = rlnorm(100,.2,.3)
#' x = stdz(x)
